...
const route = async (req, res) => {
  try {
    ...
    for (const item of result.rss.channel[0].item) {
      ...
      for (const { pattern, series, season, language, quality, offset, releasegroup } of rules) {
        ...
        const normalized = `[${releasegroup}] ${series} - S${season}E${episodeWithOffset} - ${language} - ${quality}`;
        items.push({
          title: [normalized],
          pubDate,
          enclosure,
